...
public class DatabaseHelperLesson extends SQLiteOpenHelper {

    private Context context; // поле текущего контекста

    private static final String DatabaseName = "MyLesson"; // поле названия БД
    private static final int DatabaseVersion = 1; // поле версии БД

    private static final String tableName = "myLesson"; // поле названия таблицы в БД
    ...
    private static final String columnHomework = "homework";  // поле колонки для записей домашнего задания в таблицу в БД

    // конструктор
    public DatabaseHelperLesson(@Nullable Context context) {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String query = "CREATE TABLE " + tableName + " (" + columnId + " INTEGER PRIMARY KEY AUTOINCREMENT,"
                + columnTitle + " TEXT, " + columnHomework + " Text);";
        db.execSQL(query);
    }
    ...
    public void addLesson(String title, String homework) {
        ...
        cv.put(columnHomework,homework); // добавление описания записи
    ...
    public void deleteMondayLesson() {
    ...
    public void updateLesson(String title, String homework , String id){
        ...
        cv.put(columnHomework, homework);
